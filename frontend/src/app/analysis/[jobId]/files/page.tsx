...
export default function FilesPage() {
  ...
  const filtered = useMemo(
    () => files.filter((f) =>
      !search || f.filename.includes(search) || f.mimeType.includes(search) || f.srcIp.includes(search)
    ),
    [search, files]
  )
      ...
      <div className={cn("flex-1 flex flex-col transition-all duration-300 min-w-0", sidebarOpen ? "lg:ml-56" : "lg:ml-16")}>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          <div className="p-4 border-b">
            <h1 className="text-lg font-bold mb-1">Extracted Files</h1>
            <p className="text-xs text-muted-foreground">Files extracted from HTTP response packets (per-packet payload size, no reassembly)</p>
          </div>
          <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Total Files</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{files.length}</div></CardContent></Card>
          ...
          <div className="flex-1 overflow-auto px-4">
            <div className="grid grid-cols-[100px_1fr_1fr_90px_70px_1fr] gap-3 min-w-[600px] px-4 py-2 text-xs font-medium text-muted-foreground border-b bg-background shadow-sm sticky top-0">
              <span>Time</span>
            ...
            {filtered.length === 0 && (
              <p className="px-4 py-8 text-center text-xs text-muted-foreground">
                {search ? "No files match your filter" : "No file transfers detected"}
              </p>
            )}
            {filtered.slice(0, 500).map((f) => (
              <div key={f.id} className="grid grid-cols-[100px_1fr_1fr_90px_70px_1fr] gap-3 min-w-[600px] px-4 py-2 text-xs items-center border-b border-border/50 hover:bg-accent/30">
