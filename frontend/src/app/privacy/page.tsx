...
import { Lock, Server, Scale, Mail } from "lucide-react"
...
const MATRIX: { data: string; purpose: string; processed: string; stored: string; transferred: string; control: string }[] = [
  {
    ...
    stored: "In server memory for the duration of the request; the capture file itself is never written to disk and is not part of the retained analysis result.",
  ...
  },
  {
    ...
    processed: "On the server hosting this deployment.",
    stored: "Persisted in the server-side job store (~/.packetlens/jobs.json), capped at the 20 most recent analyses with the oldest evicted automatically.",
    transferred: "None. Results are not transmitted to any third party.",
    control: "Clear Analysis (Settings → Danger Zone) discards the loaded view in your browser; server-side retention is bounded by the 20-job cap. The deployment operator can delete the store at any time.",
  },
  {
    ...
    transferred: "One-time download of the DB-IP Lite database (~125 MB) on first use. When Online GeoIP Lookups is enabled, only IP addresses not present in the local database are sent to ipwho.is.",
  ...
  },
  {
    ...
    control: "Exports contain the analysis data described above; exercise appropriate care if the capture contained sensitive material.",
  },
]
...
const PROVISIONS: { n: string; title: string; body: string }[] = [
  {
    ...
    body: "This Privacy Policy describes how PacketLens, a web application, processes data. All processing occurs on the server hosting this deployment; the operator of this deployment acts as the processor for the analysis you initiate, and you decide which captures to submit. Contact details for the deployment operator are provided in Section 10.",
  },
  {
    n: "2",
  ...
  },
  {
    ...
    body: "The application processes data solely for the purpose of analyzing the network captures you provide: parsing, protocol decoding, flow and session reconstruction, threat detection, and report generation. Where applicable legal frameworks require a basis (e.g., the GDPR), processing is performed on the basis of your consent to analyze the capture you submitted, and on the legitimate interest of operating a network analysis tool without third-party data sharing. No data is processed for advertising, profiling, or resale.",
  },
  {
    n: "4",
  ...
  },
  {
    ...
    body: "The application initiates network communications only: (1) the capture upload and job retrieval to this deployment's own server; (2) the optional ipwho.is lookup of unresolved public IP addresses, which occurs exclusively when Online GeoIP Lookups is enabled (disabled by default); and (3) a one-time download of the free DB-IP Lite GeoIP database on first use. All other processing, including DNS resolution used for hostname attribution within your own captures, is performed against data already present in the capture.",
  },
  {
    n: "6",
    title: "Storage and Retention",
    body: "Capture files are held in server memory for the duration of the request and are never written to disk. Completed analyses are persisted in the server-side job store (~/.packetlens/jobs.json), which is capped at the 20 most recent analyses with the oldest evicted automatically; each stored job is reloadable from its URL. Clear Analysis (Settings → Danger Zone) discards the loaded view in your browser but does not remove the server-side copy — server-side retention is bounded by the 20-job cap, and the deployment operator can delete the store at any time by clearing the server's ~/.packetlens directory. No data is otherwise retained after you leave the application.",
  },
  {
    n: "7",
    title: "Your Rights",
    body: "Depending on applicable law, you may have the right to access, rectify, or erase the data processed about you, to restrict or object to processing, and to data portability. Because PacketLens retains no account or personal information, exercising these rights is equivalent to choosing not to submit captures, relying on the automatic eviction of old jobs, or requesting that the deployment operator clear the server-side job store. If you believe processing violates applicable law, you may also lodge a complaint with your supervisory authority.",
  },
  {
    n: "8",
    title: "Security Measures",
    body: "Transmissions to the hosting server occur over HTTPS. Uploads are restricted to .pcap and .pcapng files, are limited to 500 MB (checked against Content-Length before the body is buffered), are rate-limited per client address, and are rejected from cross-origin callers. The server-side job store is written with file permissions 0600 inside a 0700 directory. Captures are held in memory only, minimizing exposure surface.",
  },
  {
    n: "9",
    title: "Children's Privacy",
    body: "The application is not directed at children and does not knowingly collect personal information from anyone under the age of 16. The application does not collect names, email addresses, or other identifiers of natural persons; it processes only network artifacts you choose to submit.",
  },
  {
    n: "10",
    title: "Changes and Contact",
    body: "This policy may be updated when the application's data flows change; the effective date above reflects the current revision. Questions or requests regarding this policy should be directed to the operator of the deployment you are using.",
  },
]
    ...
    <main className="min-h-screen max-w-4xl mx-auto px-4 py-12 space-y-10">
      ...
      <footer className="pt-6 border-t text-xs text-muted-foreground space-y-2">
        ...
        <p className="flex items-center gap-1.5">
          ...
          For the web deployment, direct enquiries to the operator of this deployment.
        </p>
        <p>
